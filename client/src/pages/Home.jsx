...
const Home = () => {
    // List of events fetched from the API (filtered by search term)
    const [events, setEvents] = useState([]);
    // Search input value
    const [search, setSearch] = useState('');
    // Tracks event fetch state for loading UI
    const [loading, setLoading] = useState(true);

    // Debounced fetch: waits 400ms after the user stops typing before calling the API,
    // avoiding a network request on every keystroke
    useEffect(() => {
        const timeoutId = setTimeout(() => {
            fetchEvents();
        }, 400); // 400ms debounce
        return () => clearTimeout(timeoutId); // cancel pending fetch if search changes again before timeout fires
    }, [search]);

    // Fetches events from the API, filtered by the current search term
    ...
    return (
        <div className="flex flex-col min-h-screen">
            ...
            <div className="relative bg-black text-white rounded-3xl overflow-hidden mb-12 shadow-2xl">
                ...
                {/* Gradient overlay to darken the bottom and improve text readability */}
                ...
                <div className="relative p-10 md:p-20 text-center flex flex-col items-center z-10">
                    ...
                    {/* Search bar — updates `search` state, which triggers the debounced fetch above */}
            ...
            {/* Why Choose Us / Features row — three static value-proposition cards */}
            ...
            {/* Section header with live result count */}
            ...
            {/* Conditional rendering: loading state, empty state, or the events grid */}
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {events.map(event => (
                        <div key={event._id} className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition flex flex-col">
                            {/* Event image or category placeholder, with a price badge overlay */}
                            <div className="h-48 bg-gray-200 overflow-hidden relative">
                                ...
                                {/* Price badge: shows "FREE" in green, otherwise the ticket price */}
                            ...
                            <div className="p-6 flex-grow flex flex-col">
                                ...
                                <div className="mt-auto">
                                    {/* Seat availability progress bar, width scaled by % of seats remaining */}
                                    ...
                                    {/* Link to the full event detail/booking page */}
